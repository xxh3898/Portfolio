...
화면 구현에 그치지 않고 인증 전략, API 계약, DB 모델, 테스트 자동화, REST Docs, 성능 비교, AWS 배포와 운영 검증까지 서비스 단위로 완성한 1인 개발 프로젝트입니다.
...
| 🧪 **품질 검증** | JUnit 5/MockMvc API 검증, Testcontainers 통합 테스트, Spring REST Docs, JaCoCo, Vitest, backend/frontend 분리 CI 구성 |
...
#### 🛠 기술 스택
`Java 17` `Spring Boot 3.5.12` `Spring Security` `Spring Data JPA` `QueryDSL` `MySQL 8` `Redis 7` `React 19` `Vite 8` `React Router 7` `Axios` `Recharts` `Docker Compose` `Nginx` `AWS EC2` `AWS RDS` `AWS S3` `CloudFront` `GitHub Actions` `JUnit 5` `MockMvc` `Testcontainers` `Spring REST Docs` `JaCoCo` `Vitest` `k6` `Spring Boot Actuator` `Prometheus` `Grafana` `SMTP` `Discord Webhook`
