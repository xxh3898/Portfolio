...
> CalmDesk 프로젝트에서 다량의 단건 조회로 인해 발생하는 N+1 문제를 식별하고, 벌크 조회(WHERE IN)와 메모리 매핑 구조로 리팩토링하여 25초에 달하던 150번의 쿼리 호출을 3번으로 줄이고 API 응답 속도를 0.9초로 단축했습니다. 임시방편이 아닌 데이터 흐름 설계 자체를 바꾸어 문제를 근본적으로 해결합니다.
