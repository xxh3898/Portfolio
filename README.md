...
성능 문제는 단기적 현상이 아니라 시스템 구조에서 출발해야 한다는 것을 체감했습니다. N+1 문제 같은 구조적 결함을 찾아내고 응답 속도 지연을 근본적으로 해결하는 백엔드 개발자 **조치호**입니다.
...
> CalmDesk 프로젝트에서 직원 데이터 개별 조회로 인한 N+1 문제를 식별하고, JPA `@Query JOIN FETCH`를 활용한 벌크 단일 쿼리(IN + JOIN FETCH)로 리팩토링하여 25초에 달하던 150번의 쿼리 호출을 3번으로 줄이고 API 응답 속도를 0.9초로 단축했습니다. DB 계층에서 데이터를 최적화하는 것이 WAS 메모리 부하를 원천적으로 차단하는 근본 해결임을 직접 증명했습니다.
...
#### 🚨 주요 트러블슈팅
**[성능 개선] 대시보드 API N+1 쿼리 문제 해결**
- **문제 발견**: Chrome DevTools 네트워크 탭에서 25초 지연 확인
- **원인 분석**: SQL 로그(서버 콘솔) 확인 결과 150개의 단건 쿼리가 연쇄 발생하는 N+1 구조적 결함 파악
- **해결 방안**: JPA `@Query JOIN FETCH`(IN + JOIN FETCH)로 연관 엔티티를 DB 계층에서 단일 벌크 쿼리로 한 번에 페치하도록 리팩토링, 150개의 단건 쿼리를 3개의 최적화된 벌크 쿼리로 통합
