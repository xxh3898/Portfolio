...
> CalmDesk 프로젝트에서 팀/멤버 조회 API(`GET /api/admin/team/members`) 호출 시 발생하던 N+1 문제를 식별하고, JPA `JOIN FETCH`와 `WHERE IN` 쿼리 최적화로 리팩토링하여 K6 부하 테스트 결과 **응답 속도를 377ms에서 101ms로 73% 단축**하고, **초당 처리량(RPS)을 14% 향상**시켰습니다.
