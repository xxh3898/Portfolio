...
N+1 쿼리로 인한 DB 부하와 EC2 CPU 크레딧 고갈 문제를 분석하고, 쿼리 구조 개선으로 응답 속도를 **377ms에서 101ms로 73% 단축**한 경험이 있습니다. 문제를 찾고, 근본 원인을 해결하고, 부하 테스트로 검증하는 방식으로 일하는 백엔드 개발자 **조치호**입니다.
...
**🔍 성능 병목 분석**
> SQL 로그, 인프라 상태, 부하 테스트 결과를 함께 보고 병목 원인을 추적합니다.

**🛠 근본 원인 해결**
> CalmDesk 프로젝트에서 JPA `JOIN FETCH`와 `WHERE IN` 최적화로 150개의 단건 쿼리를 3개의 벌크 쿼리로 통합해, EC2 스로틀링까지 이어지던 병목을 해결했습니다.

**📊 수치로 검증된 개선**
> k6 부하 테스트 기준 **응답 속도 377ms → 101ms (73% 단축)**, **초당 처리량(RPS) 14% 향상**을 확인했습니다.

**🤝 협업 기준 정리**
> Git 커밋 컨벤션과 GitHub 이슈/PR 템플릿을 도입해 리뷰 소요 시간을 30% 단축하고 팀 작업 가시성을 높였습니다.
...
| 🗒️ **Portfolio** | [포트폴리오](https://www.chochiho.cloud) |
...
| 구분        | 내용                                      | 기간                |
| :-------- | :-------------------------------------- | :---------------- |
| 🎓 **학력** | 세명대학교 스마트IT학부 (학점 3.33/4.5)              | 2020.03 ~ 2026.02 |
...
### 🟦 CalmDesk
> **팀 프로젝트** · 5인 Full Stack 팀 (Code808) · 이슈 관리자 담당

**다중 접속 환경에서의 실시간 상태 동기화 및 멀티테넌시 아키텍처를 적용한 기업용 B2B HR·웰빙 SaaS 통합 관리 플랫폼**
...
- 📄 **API 명세서**: [api-spec.html](./api-spec.html) (Swagger/OpenAPI 기반으로 직접 작성한 API 문서)

#### 🔧 본인 구현 주요 기능

| 기능 | 설명 |
|:---|:---|
| 💬 **실시간 채팅** | WebSocket(STOMP) 기반 실시간 1:1 / 그룹 채팅 및 커스텀 인터셉터를 활용한 소켓 연결 JWT 인증 로직 적용 |
| 🏠 **직원 대시보드** | 개인 업무 통계·정서 지수 요약 데이터 조회 및 실시간 현황 표시 화면 구현 |
| 🕐 **출퇴근 로직** | 출근·퇴근 시 관리자 대시보드 통계 갱신 로직을 @Async 비동기 이벤트로 분리하여 직원 API 응답 속도 저하 방지 |

#### 📋 전체 주요 기능
| 기능 | 설명 |
|:---|:---|
| 🤖 **AI 감정 분석** | Google Cloud STT 기반 통화 녹취 분석 + OpenAI 스트레스 지수 산출 |
| 💬 **실시간 채팅** | WebSocket(STOMP) 기반 1:1 / 그룹 채팅 및 소켓 연결 JWT 인증 (ChannelInterceptor) |
| 📣 **실시간 알림** | SSE 기반 알림 스트리밍 (휴가 승인, 긴급 상담 신청 등) |
...
#### 🚨 주요 트러블슈팅
**[근본 병목 개선] 관리자 팀/멤버 조회 API EC2 CPU 크레딧 고갈 대응 (`GET /api/admin/team/members`)**
- **문제 발견**: N+1 쿼리로 인한 DB 부하가 AWS Console 상의 **EC2(T3 Micro) CPU 크레딧 고갈 및 스로틀링 상태** 진입의 근본 원인임을 파악
- **원인 분석**: 직원 50명 조회 시 내부 루프에서 150개 이상의 추가 단건 쿼리(N+1)가 발생하는 구조적 결함을 확인
- **해결 방안**: JPA Fetch Join 및 Bulk IN 쿼리 기법을 도입하여 150개의 단건 쿼리를 3개의 최적화된 벌크 쿼리로 통합
- **결과**: **EC2 CPU 사용률 정상화 및 CPU 크레딧 고갈 문제 해결**
- **검증**: k6 부하 테스트(50 가상 유저) 결과, 응답 속도(P95) **377ms → 101ms (73% 단축)**, 초당 처리량(RPS) **14% 향상** 확인
...
### 🟩 MediFlow — W3C 병원 ERP 시스템
> **팀 프로젝트** · 6인 Full Stack 팀 · DB Lead 담당

**기존 수기/엑셀 중심의 병원 관리 비효율을 해결하기 위한 통합 ERP 시스템**

예약, 진료, 근태 관리 등 병원 운영의 전 영역을 디지털화했으며, DB 리드로서 데이터 무결성과 성능 최적화에 집중했습니다.
...
| ⚡ **쿼리 최적화** | Oracle `LISTAGG(name, ',') WITHIN GROUP (ORDER BY id)` 및 서브쿼리 활용으로 데이터 병합 과정의 N+1 문제 해결. View 도입으로 비즈니스별 복잡한 Join 성능 향상 및 SQL 복잡도 50% 감소 |
...
- **상황**: 로컬 환경 개발 시 상태값 검증 기준(A 개발자는 '1, 2, 3', B 개발자는 'Y, N, R' 기반)이 통일되지 않아 DB 병합 시 지속적인 데이터 충돌과 정합성 오류가 발생했습니다.
- **해결**: 문제 원인이 단순 문서 부재가 아닌 명시적 제약조건 적용 누락에 있음을 파악하고, 컬럼 타입을 통일한 뒤 DB 단에 `CHECK` 제약조건을 적용해 데이터 기준을 표준화했습니다.
- **결과**: 공통 더미 데이터를 재배포하여 팀 전체의 개발 환경을 동기화하고, 데이터 무결성이 훼손될 가능성을 사전에 차단했습니다.
...
### 🟨 Cubing Hub (큐빙 허브)
> **개인 프로젝트** · 1인 Full Stack 개발 · 기획/설계/개발/배포/검증 전담

**큐빙 기록·학습·랭킹·커뮤니티·피드백을 하나의 서비스 흐름으로 통합한 스피드큐빙 풀스택 웹 플랫폼**

화면 구현에 그치지 않고 인증 전략, API 계약, DB 모델, 테스트 자동화, REST Docs, 성능 비교, AWS 배포와 운영 검증까지 서비스 단위로 검증하고 마감한 1인 개발 프로젝트입니다.

- 📅 **개발 기간**: 2026.03.23 ~ 2026.04.24 (약 1개월)
...
| 기능 | 설명 |
|:---|:---|
| 🔐 **인증/인가** | 메모리 Access Token, HttpOnly Refresh Cookie, Redis Refresh Token Rotation, Access Token blacklist 기반 세션 무효화 구현 |
| ⏱️ **타이머/기록** | 스크램블 조회, 기록 저장, PB, Ao5/Ao12, penalty 수정/삭제, 게스트 로컬 기록 캐시 구현 |
| 🏆 **랭킹** | MySQL 기준 데이터와 Redis ZSET 읽기 모델을 분리해 기본 랭킹 조회 성능 개선 |
| 📚 **학습/커뮤니티** | CFOP 학습 자료, 커뮤니티 CRUD, 댓글, 다중 이미지 첨부, 공개 Q&A, 관리자 피드백 답변/공개 전환 구현 |
| 🧪 **품질 검증** | Testcontainers 통합 테스트, Spring REST Docs, JaCoCo, Vitest, backend/frontend 분리 CI 구성 |
| 🚀 **배포/운영** | S3 + CloudFront 프론트, EC2 + Nginx + Spring Boot + Redis + RDS 백엔드 분리 배포 |

#### 🏗 아키텍처 및 운영
- **Frontend**: `React` ➡️ `Vite` ➡️ `AWS S3` ➡️ `CloudFront`
- **Backend**: `Nginx` ➡️ `Spring Boot API` ➡️ `Redis` / `RDS MySQL`
- **Production URL**: `https://www.cubing-hub.com`, `https://api.cubing-hub.com`

#### 🚨 주요 트러블슈팅
**Redis ZSET 기반 랭킹 읽기 모델 분리**
- **문제 발견**: MySQL `user_pbs` 기반 V1 랭킹 조회가 `300,000` PB 기준으로 평균 `7,245.23ms`, p95 `12,429.58ms`까지 증가
- **해결 방안**: MySQL은 기준 데이터로 유지하고, 기본 랭킹 조회는 Redis ZSET 읽기 모델로 분리. 닉네임 검색과 Redis가 준비되지 않은 상태에서는 MySQL 대체 경로로 처리
- **검증 결과**: 동일 k6 시나리오에서 평균 응답 시간 `7,245.23ms → 21.10ms`, p95 `12,429.58ms → 36.94ms`, 요청 처리율 `4.21 req/s → 1,502.77 req/s`, 실패율 `0.00%`

#### ✅ 최종 검증
- backend `./gradlew test jacocoTestReport --no-daemon` 통과
- backend JaCoCo aggregate: instruction/branch 기준 누락 0건
- frontend `npx vitest run --coverage` 기준 statements/branches/functions/lines `100%`
- 실제 SMTP 송수신, 실제 AWS S3 업로드/삭제, 최종 브라우저 QA를 수동으로 확인

#### 🛠 기술 스택
`Java 17` `Spring Boot 3.5.12` `Spring Security` `Spring Data JPA` `QueryDSL` `Spring REST Docs` `MySQL 8` `Redis 7` `React 19` `Vite 8` `React Router 7` `Axios` `Recharts` `Docker Compose` `Nginx` `AWS EC2` `AWS RDS` `AWS S3` `CloudFront` `GitHub Actions` `Testcontainers` `JaCoCo` `Vitest` `k6`

---

## ✍️ 기타 활동

- **기술 블로그**: [xxh3898.tistory.com](https://xxh3898.tistory.com) — 개발 과정 및 트러블슈팅 회고 작성
