...
성능 이슈는 단순 코드 레벨을 넘어 **클라우드 인프라 자원 마비까지 직결될 수 있다**는 것을 직접 체감했습니다. N+1 쿼리로 인한 EC2 CPU 크레딧 고갈 장애를 분석·해결하며, 애플리케이션과 인프라의 연관성을 조망하는 병목 시야를 갖춘 백엔드 개발자 **조치호**입니다.

---

### 💡 핵심 역량

**🔍 성능 병목의 원인 분석 및 구조적 해결**
> CalmDesk 프로젝트에서 관리자 대시보드 통계 조회 API(`GET /api/dashboard/stats`) 호출 시 발생하던 N+1 문제를 식별하고, JPA `@Query JOIN FETCH`를 활용한 벌크 단일 쿼리(IN + JOIN FETCH)로 리팩토링하여 25초에 달하던 150번의 쿼리 호출을 3번으로 줄이고 API 응답 속도를 0.9초로 단축했습니다.

**📝 문서화 및 협업 시스템 구축**
> 개발 시작 전 기획서 ➡️ API 명세 ➡️ 시스템 아키텍처 ➡️ 트러블슈팅 등 전 과정을 문서화하는 체계적인 습관을 유지합니다. 6인 팀 프로젝트에서 Git 커밋 컨벤션, GitHub 이슈/PR 템플릿을 직접 설계하고 도입하여 구성원 간의 리뷰 소요 시간을 30% 단축시키고 원활한 협업 체계를 구축했습니다.

**⚙️ 체계적인 개발 프로세스**
> 매일 `Daily Log` 템플릿으로 목표·진행 상황·문제를 기록하고, 개념 학습 시에는 `DevQ&A` 템플릿(Why → Core Concept → 면접 답변 → Practical Tip)을 활용합니다. 개발 착수 전 기획서·ERD·시스템 아키텍처를 먼저 정리하는 순서로 프로세스를 표준화하여 작업 맥락을 유지하고 사후 추적을 용이하게 합니다.

**🤝 협업 기반 의사결정**
> 팀원 의견을 적극적으로 수용하면서, 기술적 판단이 필요한 상황에서는 근거를 제시하며 의사결정에 참여합니다.
...
| 💬 **실시간 채팅** | WebSocket(STOMP) 기반 1:1 / 그룹 채팅, 메시지 수정·삭제·읽음 처리 |
...
#### 🚨 주요 트러블슈팅
**[근본 병목 개선] 관리자 대시보드 통계 조회 API EC2 CPU 크레딧 고갈 대응 (`GET /api/dashboard/stats`)**
- **문제 발견**: Chrome DevTools 네트워크 탭에서 25초 지연 확인 → AWS Console 점검 결과 **EC2(T3 Micro) CPU 크레딧 고갈로 인한 스로틀링 상태** 진입 파악
- **원인 분석**: 직원 데이터 조회 중 **N+1 쿼리 현상** (50명 조회 시 150개 단건 쿼리 발생)이 DB 연산을 폭증시키고 CPU 자원을 과소비
- **해결 방안**: JPA `@Query JOIN FETCH`(IN + JOIN FETCH)로 연관 엔티티를 DB 계층에서 단일 벌크 쿼리로 한 번에 페치, 150개의 단건 쿼리를 3개의 최적화된 벌크 쿼리로 통합
- **결과**: 응답 속도 25초 → 0.9초 단축 (96% 향상) + **EC2 CPU 사용률 정상화 및 CPU 크레딧 고갈 현상 해결**
...
| 기능 | 설명 |
|:---|:---|
| ⏱️ **정밀 타이머** | `setInterval`의 누적 오차 문제를 해결하기 위해 `Date.now()` 기반 Delta Time 방식으로 0.001초 단위 정밀 측정 구현 |
| 🎮 **상태 머신 UX** | 실제 경기용 스택매트(Stackmat) 타이머 UX를 웹으로 완벽 이식 (`idle → holding → running` 상태 머신) |
| 🎲 **WCA 규격 스크램블** | WCA 공식 규격에 준하는 20-move 난수 생성 알고리즘 개발 |
| 📊 **대시보드 & 통계** | 날짜별 기록 자동 저장, 전체 평균/최고 기록(PB)/총 횟수 실시간 분석 |
